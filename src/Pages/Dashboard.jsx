...
export default function Dashboard() {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full p-5 gap-6">
      ...
      {/* Badge */}
      <div className="shrink-0 px-3 py-2 rounded-full bg-sky-500/10 border border-sky-500/20 flex items-center gap-2">
        <span className="w-1.5 h-1.5 rounded-full bg-sky-400 animate-pulse" />
        <span className="text-sky-300 text-xs font-medium">Portfolio Manager</span>
      </div>
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 text-sm font-medium shrink-0 ${
                active
                  ? 'bg-gradient-to-r from-sky-500/20 to-cyan-500/15 border border-sky-500/30 text-white'
