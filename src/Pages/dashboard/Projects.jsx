...
const InputField = ({
  ...
  <div className="space-y-1.5">
    <label className="text-xs text-sky-300/70 uppercase tracking-wider font-medium">
      {label}
    </label>
    <input
      type={type}
      ...
      required={required}
      className="w-full bg-[#0d0d22] border border-white/10 rounded-xl px-4 py-2.5 text-gray-200 placeholder-gray-600 text-sm outline-none focus:border-sky-500/60 focus:ring-1 focus:ring-sky-500/20 transition-all"
    />
  </div>
);
        ...
        {project.tech_stack?.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-3">
            {project.tech_stack.map((t) => (
              <span
                key={t}
                className="px-2 py-0.5 rounded-full bg-sky-500/15 border border-sky-500/25 text-sky-300 text-xs"
              >
                {t}
              </span>
            ))}
        ...
        <div className="mt-auto flex items-center justify-between gap-2 pt-2 border-t border-white/8">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-sky-500/25 text-sky-400 hover:bg-sky-500/10 text-xs transition-colors"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="sm:col-span-2 space-y-1.5">
          ...
          <textarea
            ...
            rows={3}
            className="w-full bg-[#0d0d22] border border-white/10 rounded-xl px-4 py-2.5 text-gray-200 placeholder-gray-600 text-sm outline-none focus:border-sky-500/60 focus:ring-1 focus:ring-sky-500/20 transition-all resize-none"
          />
        </div>
  ...
  return (
    <div className="space-y-6z ">
      ...
      <div className="flex items-center justify-between gap-4 mb-6 flex-wrap">
        ...
        >
          <div className="absolute -inset-0.5 bg-gradient-to-r from-[#0ea5e9] to-[#06b6d4] rounded-xl opacity-50 blur group-hover:opacity-80 transition duration-300" />
